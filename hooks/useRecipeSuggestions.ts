...
export function useRecipeSuggestions(ingredients: string[] | null) {
  ...
  const fetchRecipesInternal = useCallback(async (currentOffset: number) => {
    ...
    try {
      ...
      const result = await fetchRecipesByIngredients(ingredients, currentOffset);
      setRecipes(result);
  ...
  // Effect for initial fetch when ingredients change
  useEffect(() => {
    if (ingredients && ingredients.length > 0) {
      setRecipes(null); // Clear previous recipes
      setOffset(0); // Reset offset for new ingredients
    ...
    // to only trigger on ingredient change.
